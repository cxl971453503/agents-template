...
1. 输出前确认：变更是否仅限于当前 Task ID 的目标范围，契约/架构/任务范围变更时是否已同步文档，新增或修改的类型/字段/函数是否已补充中文注释。
2. 遇到模糊点时：暂停实现，向用户或上一阶段角色确认后再继续。

## 护栏

- 输入护栏：任务必须有明确的 Task ID、目标、输入、输出和完成定义；涉及文件删除或环境变更时需用户确认。
- 输出护栏：变更必须与 `docs/architecture.md` 和 `docs/api-contracts.md` 一致；不得引入未在任务范围中声明的依赖或框架。
